...
class MainWindow(QMainWindow):
	...
	def setupConnections(self):
		...
		self.daughterCB.currentIndexChanged.connect(self.daughterChanged)
	...
	def daughterChanged(self):
		if self.daughterCB.itemData(self.daughterCB.currentIndex()) == '7i77':
			for i in range(5):
				getattr(self, 'stepGen_' + str(i)).setVisible(False)
		else:
			for i in range(5):
	...
	def buildCB(self):
			...
			for item in buildcombos.setupCombo('daughter'):
					self.daughterCB.addItem(item[0], item[1])
